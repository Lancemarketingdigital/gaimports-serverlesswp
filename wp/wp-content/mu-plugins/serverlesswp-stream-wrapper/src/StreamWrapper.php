...
class StreamWrapper
{
    ...
    public static function pushLocalFileStatus(string $absolutePath, bool $deleteLocal = true): string
    {
        ...
        if (!self::$adapter->put($key, $contents)) {
            ...
            trigger_error(
                "serverlesswp-stream-wrapper: failed to push uploaded file '{$key}' to remote storage",
                E_USER_WARNING,
            );
    ...
    public function stream_open(string $path, string $mode, int $options, ?string &$opened_path): bool
    {
        ...
        if ($baseMode === 'x' || $baseMode === 'x+') {
            if (self::$adapter->exists($this->storageKey)) {
                ...
                if ($options & STREAM_REPORT_ERRORS) {
                    trigger_error("serverlesswp-stream-wrapper: remote file '{$this->storageKey}' already exists", E_USER_WARNING);
                }
                return false;
            }
            $this->isDirty = true;
        ...
        if ($baseMode === 'r' || $baseMode === 'r+') {
            ...
            if ($existing === null) {
                ...
                $this->buffer = null;
                if ($options & STREAM_REPORT_ERRORS) {
                    trigger_error("serverlesswp-stream-wrapper: cannot open remote file '{$this->storageKey}'", E_USER_WARNING);
                }
                return false;
            }
        ...
        if (in_array($baseMode, ['a', 'a+', 'c', 'c+'], true)) {
            ...
            if ($existing === null && !$this->absentAtOpen) {
                ...
                if ($options & STREAM_REPORT_ERRORS) {
                    trigger_error(
                        "serverlesswp-stream-wrapper: cannot read '{$this->storageKey}' to open it for "
    ...
    private function commit(string $contents): void
    {
        ...
        if (!$this->isAppend) {
            ...
            trigger_error(
                $this->absentAtOpen
                    ? "serverlesswp-stream-wrapper: dropped write to '{$this->storageKey}' — another "
        ...
        if ($replayed === false) {
            $this->recordFailedWrite();
            trigger_error(
                "serverlesswp-stream-wrapper: dropped append to '{$this->storageKey}' after "
    ...
    public function rmdir(string $path, int $options): bool
    {
        ...
        if (!str_contains($key, '/')) {
            trigger_error(
                "serverlesswp-stream-wrapper: refusing rmdir on root storage target '{$key}'",
                E_USER_WARNING,
            );
            return false;
        }
        ...
        if ($children !== []) {
            trigger_error(
                "serverlesswp-stream-wrapper: rmdir('{$resolved}') failed — directory not empty",
                E_USER_WARNING,
            );
            return false;
        }
        ...
        if (!self::$adapter->delete($marker)) {
            trigger_error(
                "serverlesswp-stream-wrapper: failed to delete directory marker '{$marker}'",
                E_USER_WARNING,
            );
            return false;
        }
    ...
    private function openPassthrough(string $resolved, string $mode, int $options, ?string &$opened_path): bool
    {
        ...
        if ($handle === false) {
            if ($options & STREAM_REPORT_ERRORS) {
                trigger_error("serverlesswp-stream-wrapper: failed to open '{$resolved}' with mode '{$mode}'", E_USER_WARNING);
            }
            return false;
        }
